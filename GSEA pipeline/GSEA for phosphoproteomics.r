...
# calculate ranks
c(phos_results_table, phos_ranks_msd) %<-% calc_ranks(
...
save_gsea_plot(plot_metric(phos_results_table, "signed_p"), paste0(output_dir, "phos_ranks_signed_p.png"))
...
phos_signed_p_h <- run_and_plot_fgsea(h_gs_gsymbol, phos_ranks_signed_p, scoreType="pos")

# save figures
save_sig_enrichment_plots(results = phos_signed_p_h,
                          genesets = h_gs_gsymbol,
                          ranks = phos_ranks_signed_p,
                          path = output_dir,
                          name = "phos_signed_p_h")

# save complete results
save(phos_signed_p_h, file=paste0(output_dir, "phos_signed_p_h", ".RData"))
